...
void setup(void) {
  ...
  if (mpu.begin()) Serial.println(F("MPU6050: Init success!"));
  ...
  if (bmp.begin()) Serial.println(F("BMP180: Init success!"));
  else Serial.println(F("BMP180: Init failed."));
...
void WriteToCard() {
  ...
  double entry[10] = { millis() / 1000.000, aX, aY, aZ, altitude, temp, pressure, wX, wY, wZ };
  if (file) {
    for (int i = 0; i < 10; i++) {
      file.print(entry[i]);
      if (i < 9) file.print(",");
    ...
    Serial.println("Error opening file: data.csv");
  }
}
